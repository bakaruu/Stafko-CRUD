    ...
    useEffect(() => {
        const fetchProject = async () => {
            try {
                ...
                // Haz una solicitud a tu API de Directus para obtener la información del proyecto
                const response = await axios.get(`http://localhost:8055/items/projects/${id}`, config);
                const projectData = response.data.data;

                // Construir la URL de la imagen
                const photoUrl = `http://localhost:8055/assets/${projectData.image}`;

                // Guarda la información del proyecto en el estado
                setProject({
                    ...projectData,
                    photoUrl
                });
...
export default ProjectInfoUser;
